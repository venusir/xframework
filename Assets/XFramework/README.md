...
R3 通过 NuGetForUnity 安装，而非 UPM。点击 `XFramework -> Install Dependencies` 后，脚本会自动在 `Assets/packages.config` 中添加 R3 及其依赖项。然后请点击菜单栏 `NuGet -> Restore` 来下载 R3。

**如果先添加了 XFramework 导致编译报错：**

在 Unity Editor 中，点击菜单栏 `XFramework -> Install Dependencies`，脚本会自动安装所有依赖（UPM 包写入 `manifest.json`，NuGet 包写入 `packages.config`），然后 Unity 会重新编译。
