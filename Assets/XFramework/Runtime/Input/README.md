...
- **长按时长 / 绑定 / 重绑定 / ActionMap / 设备检测**：全局共享，不区分玩家

> **已知限制（最小档）**：手柄按连接顺序分配，热插拔后玩家与手柄的对应关系可能变化；
> 不支持双人键鼠；无 InputUser 级设备配对；`playerId > 0` 的读取为控件级原始值，不应用绑定处理器。
> 完整档（每玩家 action 实例 + 设备配对）待需求明确后另行实现。
