...
namespace XFramework.XLock
{
    ...
    public static class LockableExtensions
    {
        ...
        /// </summary>
        public static LockHandle AddLock(this ILockable self, int lockType, object lockObj)
            => LockManager.AddLock(self, lockType, lockObj);
        ...
        /// <summary>
        /// 订阅 <c>this</c> 的锁定事件。
        /// <para>全局锁（<see cref="LockManager.Global"/>）的锁定也会触发此回调。</para>
        ...
        public static IDisposable OnLocked(this ILockable self, Action<int> handler)
            => LockManager.OnLocked(self, handler);

        /// <summary>
        /// 订阅 <c>this</c> 的解锁事件。
        /// <para>全局锁（<see cref="LockManager.Global"/>）的解锁也会触发此回调。</para>
        /// <para>返回 <see cref="IDisposable"/>，调用 <c>Dispose()</c> 可取消订阅。</para>
        /// </summary>
