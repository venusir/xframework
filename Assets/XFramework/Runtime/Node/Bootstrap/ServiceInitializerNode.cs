...
{
    /// <summary>
    /// 服务初始化节点。在启动阶段统一管理需要参与相位分组调度的模块（如 AssetManager 等）的生命周期。
    /// <para>LockManager、MessageManager 等纯静态服务已通过 [RuntimeInitializeOnLoadMethod] 自动管理生命周期，无需在此注册。</para>
    /// <para><see cref="AssetBootstrapNode"/> 等引导节点实现了 <see cref="XPipeline.IPhaseStage"/>，
    /// 在启动管线的相位分组中异步初始化 <see cref="XAsset.AssetManager"/> 等服务。</para>
    ...
    public class ServiceInitializerNode : EntityNode
    {
        ...
        /// <para>此方法在 <see cref="OnAwake"/> 中调用，早于启动管线的执行。</para>
        /// </summary>
