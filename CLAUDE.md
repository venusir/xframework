...
- **测试:** 完成逻辑后编写单元测试并提供验证步骤。全量测试由用户在 Unity 编辑器 Test Runner 中手动启动;Claude 可用 `Tools/run-tests.ps1` 以 `-testFilter` **定向**跑本次改动相关的 fixture 自测,但不得以命令行全量跑替代人工验证——全量 PlayMode 跑存在跨 fixture 静态状态泄漏噪音(各模块静态门面相互污染),汇总不可作为门禁
